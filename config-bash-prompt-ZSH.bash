...
# psvar[8] - dir[123] segment for rightmost directories in path
# psvar[8] - First prompt segment having a single background color
# psvar[9] - Next prompt segment having a single background color
#
PSV_VI=1
PSV_ID=2
PSV_RET=3
...
function precmd
        ...
        then
                ...
                if [[ ! -w $PWD ]];
                then
                        psvar[${PSV_LOCK}]="$(echo $PROMPT_LOCK)"
                fi
        fi
        ...
        then
                ...
                then
                        psvar[${PSV_VCS}]="$(echo ${PROMPT_GIT_BRANCH} $vcs_info_msg_0_)"
                else
                        psvar[${PSV_VCS}]=''
                fi
        fi
        ...
        if [[ ! -z $_dirNPromptUsed ]];
        then
                #psvar[${PSV_DIRN}]="$(_lastDirEntries $_dirEntriesToUse)"
                psvar[${PSV_DIRN}]="$(_lastDirEntries 2)"
        fi
        }
...
function _commonPrompt
        ...
        *)
        ...
        do
                ...
                dir2)
                        _seg+='${psvar[${PSV_DIRN}]}'
                        _checkHiddenSegment="dirN"
                        _dirNPromptUsed=true
                        ;;
                dir3)   _seg+="%3d"                  ;;
...
#DM     fi
        ...
        then
                ...
                id)
                        ...
                        ;;
                dirN)
                        psvar[$_segIndex]="%(${PSV_DIRN}V?${_seg}?)"
                        ;;
                *)
                        psvar[$_segIndex]="$_seg"
                        ;;
